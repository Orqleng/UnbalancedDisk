...
class UnbalancedDisk(gym.Env):
    ...
    def __init__(self, umax=3., dt = 0.025, render_mode='human'):
        ...
        #self.action_space = spaces.Box(low=-umax,high=umax,shape=(1,), dtype=np.float32) #continuous
        self.action_space = spaces.Discrete(5,) #discrete
        low = [-np.pi,-40.]
        high = [np.pi,40.]
        self.observation_space = spaces.Box(low=np.array(low,dtype=np.float32),high=np.array(high,dtype=np.float32),shape=(2,))
        #self.reward_fun = lambda self: np.exp(-(self.th%(2*np.pi)-np.pi)**2/(2*(np.pi/7)**2))# + 0.1 * np.exp(-self.omega**2 / 2) #example reward function, change this!
        weight_omega, weight_action = 0.2, 1#1e-3
        self.observation_space = spaces.Box(low=np.array(low,dtype=np.float32),high=np.array(high,dtype=np.float32),shape=(2,))
        #self.reward_fun = lambda self: np.exp(-(self.th%(2*np.pi)-np.pi)**2/(2*(np.pi/7)**2))
    
        self.reward_fun = lambda self: -90*(np.pi-np.abs(self.th))**2 - weight_omega *self.omega **2 - weight_action*self.u**2
